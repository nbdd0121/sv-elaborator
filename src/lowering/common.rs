...
pub fn name_of_mut(item: &mut HierItem) -> Option<&mut Ident> {
    match item {
        ...
        HierItem::NetDecl(decl) => Some(&mut Rc::get_mut(decl).unwrap().name),
        HierItem::FuncDecl(decl) => Some(&mut Rc::get_mut(decl).unwrap().name),
        HierItem::TaskDecl(decl) => Some(&mut Rc::get_mut(decl).unwrap().name),
        HierItem::ContinuousAssign(..) => None,
        HierItem::Always(..) => None,
        HierItem::Other(..) => None,
...
pub fn name_of(item: &HierItem) -> Option<&Ident> {
    match item {
        ...
        HierItem::NetDecl(decl) => Some(&decl.name),
        HierItem::FuncDecl(decl) => Some(&decl.name),
        HierItem::TaskDecl(decl) => Some(&decl.name),
        HierItem::ContinuousAssign(..) => None,
        HierItem::Always(..) => None,
        HierItem::Other(..) => None,
        HierItem::InstancePart { .. } => unreachable!(),
